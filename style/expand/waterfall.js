...
;(function( $, window, document, undefined ) {
        ...
        defaults = {
            ...
            callbacks: {
                ...
                loadingError: function($message, xhr) {
                    /*if ( xhr == 'end') {
                        $message.html('不要太用力！后面没有了！');
                    }else{
                        $message.html('小伙子！~~~我报错了！你晓得不！');
                    }*/
                },
                 ...
                 */
                renderData: function (data, dataType,eyetype) {
                    var tpl,
                        template;
                    ...
                    if ( dataType === 'json' ||  dataType === 'jsonp'  ) { // json or jsonp format
                        ...
                        var newstring = '';
                        var length = newdata == null ? 0 : newdata.length;
                        if(eyetype == 1){
                            for (var i = 0 ; i < length; i++) {
                                var tdata = newdata[i];
                                newstring = newstring + '<div class="single_item">'+
                                                            '<div class="img"><a href="'+tdata.itemurl+'" target="_blank"><img src="/data/attachment/recordimg/'+tdata.image+'" alt=""></a></div>'+
                                                            '<div class="u">'+
                                                                '<div class="u_img">'+
                                                                    '<a href="/home.php?mod=space&uid='+tdata.uuid+'&do=profile" target="_blank"><img src="/uc_server/avatar.php?uid='+tdata.uuid+'&size=big&gender='+tdata.usex+'" alt="'+tdata.uname+'"></a>';
                                                                if(tdata.isVIP){
                                                                        newstring = newstring + '<i class="vip">&nbsp;</i>';
                                                                    }
                                            newstring = newstring +'</div>'+
                                                                '<div class="u_info">'+
                                                                    '<div class="icon"><span class="u_n"><a href="/home.php?mod=space&uid='+tdata.uuid+'&do=profile" target="_blank">'+tdata.uname+'</a></span>';
                                                                    if(tdata.isPhone=='1'){
                                                                        newstring = newstring + '<span class="i1">&nbsp;</span>';
                                                                    }
                                                                    if(tdata.isidNo=='1'){
                                                                        newstring = newstring + '<span class="i2">&nbsp;</span>';
                                                                    }
                                                                    if(tdata.isPower=='1'){
                                                                        if(tdata.groupid=='22'){
                                                                            newstring = newstring + '<span class="i3">&nbsp;</span>';
                                                                        }else{
                                                                            newstring = newstring + '<span class="i5">&nbsp;</span>';
                                                                        }
                                                                    }
                                            ...
                                            newstring = newstring + '</div>'+
                                                            ...
                                                            '</div>'+
                                                            '<div class="d"><span class="d1">'+tdata.lookNum+'</span><span class="d2">'+tdata.msgNum+'</span><span class="d3">'+tdata.likeNum+'</span></div>'+
                                                        '</div>';
                            };
                        }else{
                            for (var i = 0 ; i < length; i++) {
                                var tdata = newdata[i];
                                newstring = newstring + '<a href="'+tdata.itemurl+'" title="'+tdata.title+'" class="item"><img src="/data/attachment/recordimg/'+tdata.image+'" alt="'+tdata.title+'"></a>';
                            };
                        }
                        return newstring;
    ...
    Waterfall.prototype = {
        ...
        _handleResponse: function(data, callback) {
            var self = this,
                options = this.options,
                content = $.trim(options.callbacks.renderData(data, options.dataType,options.eyetype)),
                $content = $(content),
        ...
        _doScroll: function() {
            ...
            if(self.options.eyetype == 2){
                jQuery('.uinfominimg').bind('scroll', function() {
                    if ( scrollTimer ) {
                        clearTimeout(scrollTimer);
                    }
