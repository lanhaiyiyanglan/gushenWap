	   ...
	   $.ajax({
		...
		success:function(data){
			...
			for(var i=0;i<data.data.length;i++){
				...
				html+='<div class="perIncon"><img src="'+data.data[i].url+'"></div>';
				html+='<div class="personalText">';
				html+='<div class="namePlace">';
				html+='<div class="friendName"><span class="num_name">'+data.data[i].user_name+'</span><span class="roler">';
				if(data.data[i].role==1){
				  html+='高手';
				}else{
				  html+='金主';
				}
				html+='</span></div><input type="hidden" value="'+data.data[i].user_id+'"></li>';
			}
			$('.friendList').append(html);
			$("#friendList > li").click(function(){//选中样式
